...
def generate():
    """Generate and save a synthetic IoT traffic dataset.

    Creates a learnable binary classification dataset using sklearn, enriches
    it with realistic column scaling and a multi-class ``attack_type`` column,
    then writes the result to ``data/raw/synthetic_iot_traffic.csv``.

    :returns: The file path where the dataset was written.
    """
