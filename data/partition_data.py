...
def report(nodes, label):
    """Print a summary of each node's partition statistics.

    :param nodes: List of per-node DataFrames.
    :param label: Label string for the partition type (e.g. ``"IID"``).
    :returns: One tuple per node: (index, n_rows, attack_rate, top_types_str).
    """
...
def main():
    """Partition the full dataset into IID and non-IID node splits and save CSVs.

    Creates the directory structure under ``data/nodes/``, holds out a global
    test set, then writes per-node CSV files for both IID and non-IID splits.
    """
