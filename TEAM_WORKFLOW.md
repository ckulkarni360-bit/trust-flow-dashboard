...
# create + push each person's working branch so the other person can
# just `git checkout <branch>` right after cloning
git checkout -b nitesh/federated-core
git push -u origin nitesh/federated-core

git checkout -b chaitanya/trust-robustness
...
2. Commit and push regularly to your own branch:
   ...
   ```
3. Open a Pull Request into `main` when your phase is verified working (include your
...
4. **Merge order:** Track A's Phase 3 should merge to `main` first (Track B's attack
   comparison script imports the FedAvg strategy as the control condition). After that,
   either track can merge independently.
5. Rebase on `main` periodically so you're not diverging for weeks:
   ```
   git fetch origin
...
| Week after | Merge both into `main`, run the full comparison suite together, start dashboard + write-up |
