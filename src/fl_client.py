...
class NodeClient(fl.client.NumPyClient):
    """Flower client representing a single network node/device in the federated network.

    Trains a local model replica on the node's traffic data slice and returns
    model parameters without sharing raw network traffic.
    """

    def __init__(self, node_id: str, node_df: DataFrame, scaler, boost_factor: float = 1.0):
        ...
        :param node_df: This node's local traffic dataset partition.
        :param scaler: Fitted StandardScaler used to normalise feature columns.
        :param boost_factor: Update-magnitude multiplier. Values > 1.0
            amplify this node's gradient to simulate a model-replacement
            style attack. Use 1.0 for an honest node.
        """
    ...
    def get_parameters(self, config):
        """Return the current local model weights as a list of ndarrays.

        :param config: Configuration dict (unused, required by Flower API).
        :returns: Current model weights (coefs_ + intercepts_).
        """
        return mu.get_weights(self.model)

    def fit(self, parameters, config):
        """Train the local model on this node's data for a number of epochs.

        :param parameters: Global model weights to start from.
        :param config: Training config; expects ``"local_epochs"`` key.
        :returns: ``(updated_weights, num_examples, metrics_dict)`` where
            metrics_dict contains ``"node_id"``.
        """
    ...
    def evaluate(self, parameters, config):
        """Evaluate the global model on this node's local validation slice.

        :param parameters: Global model weights to evaluate.
        :param config: Configuration dict (unused, required by Flower API).
        :returns: ``(loss, num_examples, metrics_dict)`` where metrics_dict
            contains ``"accuracy"`` and ``"f1"``.
        """
        mu.set_weights(self.model, parameters)
